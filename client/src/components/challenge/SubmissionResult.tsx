...
export const SubmissionResult: React.FC<SubmissionResultProps> = ({
  ...
  const passedTests = submissionResult.testResults?.filter((r: any) => r.passed).length || 0;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {!hasError && submissionResult.testResults && submissionResult.testResults.length > 0 && (
        <div>
          <h3 className="text-white font-medium mb-4">Test Cases ({passedTests}/{totalTests} passed)</h3>
                    ...
                    <div className="space-y-2 text-xs">
                      {testCase && !testCase.isHidden && (
                        <>
                          <div>
                            ...
                            <span className="text-gray-300 font-mono">{testCase.input}</span>
                          </div>
                          <div>
                            <span className="text-gray-400">Expected: </span>
                            <span className="text-gray-300 font-mono">{testCase.output}</span>
                          </div>
                          <div>
                      ...
                      {result.error && (
                        <div>
                          ...
                          <span className="text-red-300 font-mono text-xs">{result.error}</span>
                        </div>
                      )}
                    </div>
                  )}
      ...
      {hasError && (
        <div className="p-4 bg-red-900/20 border border-red-500/30 rounded-lg">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="text-red-400 font-medium mb-1">Submission Error</div>
